...
class WeaviateClient:
  ...
  def add_data(self, data):
    try:
      ...
      self.client.data_object.create(data_object=weaviate_data,
                                     class_name="Message")
    except Exception as e:
      logging.error("Error adding data to Weaviate: %s",
                    str(e))  # Logging the error
      traceback.print_exc()
      ...
      async def on_message(message):
        try:
          ...
          if bot.user.mentioned_in(message):
            ...
            ram_messages = " ".join(
                [past_message['message'] for past_message in past_messages])
